...
def test_get(d: date, stadium: int, race: int):
    ...
    # preparation
    mock_html_file = f"oddsk.rno={race}&jcd={stadium:02d}&hd={d.strftime('%Y%m%d')}.html"  # noqa
    expected_file = f"expected_oddsk.rno={race}&jcd={stadium:02d}&hd={d.strftime('%Y%m%d')}.json"  # noqa
